...
const now       = Date.now();
const WINDOW_MS = 10 * 60 * 1000; // 10-min window (cron fires at exact lock time)
...
for (const m of toLock) {
  ...
  await flagRef.set({ backedUp: true, sentAt: admin.firestore.FieldValue.serverTimestamp(), matchId: m.matchId });
